...
const ProgramInfo: React.FC<ProgramInfoProps> = ({ channel, program, targetLanguage }) => {
  const [translatedProgram, setTranslatedProgram] = useState<{ title: string; desc: string } | null>(null);
  const [isTranslating, setIsTranslating] = useState(false);

  useEffect(() => {
    if (program && targetLanguage !== 'Japonês') {
        ...
        const translate = async () => {
            ...
            try {
                const [translatedTitle, translatedDesc] = await Promise.all([
                    geminiService.translateGenericText(program.title, targetLanguage),
                    geminiService.translateGenericText(program.desc || '', targetLanguage)
                ]);
